...
func Types() []EventType {
	return []EventType{
		...
		{Type: TypeTeamTopology, Source: SourceHost,
			Doc: "the resolved shape of a team, written once at boot, after every agent's own " +
				"session.ready/session.policy pair (docs/policy-record.md §3). Carries no agent " +
				"field of its own — it describes the team, not one machine, the same scope " +
				"session.start and session.end already use for one. cpu_quota_percent is shared " +
				"with resource.oom, resource.summary and session.policy rather than duplicated, " +
				"carrying the team-wide cap here instead of one machine's",
			Fields: []Field{
		...
		{Type: TypeSessionErasure, Source: SourceHost,
			Doc: "every field somewhere in this chain known to carry guest-influenced or " +
				"operator-supplied content was replaced with a fingerprint of what was there — " +
				"its own sha256 — by kelyfos sessions erase (P7-5, D61); docs/retention.md §5 has " +
				"the full field list and why each field is or is not covered. The chain is " +
				"rewritten and rehashed in full, so the digest of every event after the first one " +
				"touched is different from what it was before this ran; the chain still verifies, " +
				"and this event is the last one, appended once the rewrite is complete. It is not " +
				"necessarily the last event a reader sees when checking whether the session ended " +
				"cleanly — kelyfos verify looks for session.end anywhere in the chain, not only as " +
				"the last event, so an erasure does not make a cleanly-closed session read as cut " +
				"short",
			Fields: []Field{
