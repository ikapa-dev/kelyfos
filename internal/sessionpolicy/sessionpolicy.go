...
// Ports is the egress allowlist's effective port coverage. egress.Policy.
// Ports has no caller yet (P7-4) and internal/egress/proxy.go's own runtime
// default is [80, 443] whenever Ports is unset, so that is the honest value
// to record for any machine with a network at all — recording nothing would
// misstate a real, enforced default as an absence.
