...
# Everything below is one function, called on the last line. Bash reads a
# script lazily, by byte offset, as it executes; a run takes about nine
# minutes and CLAUDE.md asks every session to pull and push the same day, so
# the file changing on disk mid-run is ordinary rather than exotic — and it
# ended a run that had just printed "Job succeeded" with a syntax error and
# no summary, which reads as a failed check. Found by an accidental mid-run
# pull, not by inspection. Wrapping the body makes bash parse the whole file
# before it executes any of it, so the copy that started is the copy that
# finishes.
main() {
