...
## This session
E1–E3 closed and **v0.6 released**: a generated reference CI fails on drift for,
`llms.txt` at spec v2, CI-run recipes, a docs exam that found ten defects, then
the F-D33 batch and the seam check.
...
## Blocked / debts
- P4-4, P4-5 [BLOCKED]. `idle_timeout` refused (F-D20; F-D22). IMAGE_DIR per-arch not per-flavor: parked. A live Claude Code session vs serve-mcp needs John.
