...
after it is done (F-D49): the ignored `workspace` key says so now, a plugin tool
cannot shadow a built-in, and `plugin.call` records its arguments redacted the
way `mcp.host.call` does.

## Blocked / debts
- P4-4, P4-5 [BLOCKED]. `idle_timeout` refused (F-D20; F-D22). IMAGE_DIR per-arch not per-flavor: parked.

## The E4→E5 seam: a platform-refresh pair (John's ruling) — next
Both F-D35 premises falsified. Before E5-0:
