...
## Now
E3-4 — the integration guide: KelyfOS from Python and JS, the MCP bridge, the
shim, and the mistakes the build log already knows about. Then E3-5's docs-only
exam, the E3 exit checklist, and tag v0.6.
...
conformance tested) + `llms-full.txt`. E3-3: seven cookbook recipes, each run
before being written down — **7 passed, 0 failed**. `llms-full.txt` is **54,306
tokens**, 27% of a 200k window (F-D29, F-D30).
