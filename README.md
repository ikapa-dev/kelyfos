...
| [`docs/cookbook.md`](docs/cookbook.md) | seventeen recipes that work: run one, allowlist a domain, fork, build a team, point a client at it, write a plugin, verify the log, pause and resume, review a diff, forward a port, draw a team's topology, follow a running team through a live-refreshed export |
