...
// What both files carry instead is the newest release named in CHANGELOG.md,
// which changes only when a release section is cut (D50) and is therefore the
// same on every commit in between. The README has no status block to read it
// from — its release badge is rendered by GitHub — so the changelog is the one
// committed place the release is written down.
func llmsIndex(fullTokens int, release string) string {
	...
	b.WriteString("The newest release is " + release + "; CHANGELOG.md names what each one changed.\n\n")
...
// changelogRelease is the newest release CHANGELOG.md has a section for, in the
// form the heading uses: "v1.3.0 (2026-09-02)". The first "## v<...> — <date>"
// heading wins, which is the file's own convention (tools/changelog.py cuts a
// release's notes from the section matching its tag); an "Unreleased" heading,
// if one is ever added, does not match and is skipped.
var changelogHeading = regexp.MustCompile("(?m)^## (v[0-9][^ ]*) — ([0-9]{4}-[0-9]{2}-[0-9]{2})$")

func changelogRelease(repo string) (string, error) {
	...
	if m == nil {
		return "", fmt.Errorf("CHANGELOG.md has no \"## v<version> — <date>\" heading to name the release")
	}
...
func llmsFull(repo string, release string) (string, error) {
	var b strings.Builder
	...
	b.WriteString("Apache-2.0. Generated by `make docs`: every page below is the committed file,\n" +
		...
		"drift. The release this describes is " + release + ", the newest section of CHANGELOG.md.\n\n")
