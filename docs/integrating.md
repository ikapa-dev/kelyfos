...
**The write is atomic.** It goes to a temporary file beside the target, is
fsynced, and is renamed over it — this is a read-modify-write of a file another
program may have open, and a rename is the only atomic replacement available. A
file that will not parse is refused before anything is written and is left
exactly as it was.
