...
1. **The image is validated before it is read.** Every entry is enumerated and
   ...
   look like they stay inside compose into one that does not. An acyclic chain this host
   cannot follow to its end is refused as well, rather than assumed harmless:
   the kernel gives up at 40 links, but `os.path.realpath`, Node's
   `fs.realpathSync` and Go's own `filepath.EvalSymlinks` all follow much
   further or without limit, and those are the tools that meet the link once it
   is sitting in somebody's project. A *cycle* is allowed through, because
   nothing can follow one anywhere — `cat` and `EvalSymlinks` both give up, and
   `realpath`, the one resolver with no hop limit, detects the loop — and
   refusing a whole image over a link that reaches nothing would cost a session
   for no gain.

   The resolution asks the **filesystem** whether two spellings are one name,
   rather than canonicalising them here. That is not a detail: the destination
   decides, and it decides differently in more ways than can be listed. The
   macOS home directory this project's primary platform shares into its Linux
   VM treats `Docs` and `docs` as one name *and* the NFC and NFD spellings of
   `café` as one file, while `/tmp` on the same machine treats both pairs as
   two. Case folding and Unicode normalization were each found, one at a time,
   to defeat a canonical-form check that had been written to close the other;
   the tree is therefore built and then asked, which answers for every
   equivalence the filesystem implements including the ones nobody here has
   heard of. Checking is bounded by a per-image budget, because how many links
   there are and how long each target is are both the guest's to choose.
...
| guest → credentials | injection at the proxy | active; and since v1.1 the proxy serves the guest's address alone — it refuses any other peer and the ruleset drops packets for the host's TAP address that did not arrive on the TAP, because binding a host address is not the same as being unreachable from the host |
| guest → audit record | host-side, hash-chained | active, including under `kelyfos shim`; and since v1.1 it fails closed — the first failed append is final, and every loop that keeps a machine alive stops that machine rather than running it unrecorded |
| guest → host filesystem, via the workspace disk | the image is enumerated and validated before it is read, every symlink is created in the staging tree first and every chain is then resolved by asking that tree — the filesystem the workspace is going to, which is the only thing that can say whether two spellings are one name — every file is checked against the size its inode records, twice, and the writes go through an `os.Root` | active since v1.0. **This row did not exist until an external audit found what its absence cost** — see below |
