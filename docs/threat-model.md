...
1. **The image is validated before it is read.** Every entry is enumerated and
   ...
   look like they stay inside compose into one that does not. A chain this host
   ...
   is sitting in somebody's project. The comparison folds case, because the
   filesystem the tree is renamed onto may fold it too — the macOS home
   directory this project's primary platform shares into its Linux VM does —
   and a component spelled `D1` that is really the symlink `d1` would otherwise
   read as an ordinary directory.
