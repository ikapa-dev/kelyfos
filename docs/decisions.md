...
**Why:** P6-4. PLAN-FEATURES.html §4’s parked entry and its Azure SRE Agent source; D6 for why the value stays on the host; F-D33 for the standard this project holds itself to when describing what the proxy can and cannot see. *Revisit when* either blocking question has an answer, or when a guest→host request channel exists for a non-team sandbox for some other reason.
