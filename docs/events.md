...
one — and `kelyfos team up`'s, which covers a whole rig because a team has one
recorder for all of it. `kelyfos serve-mcp` has no such loop, so each sandbox it
creates gets a watcher of its own, started where the machine is registered.

When it fires, the machine is stopped and you are told which event was lost and
why:

```
kelyfos: the flight recorder stopped at event 41: no space left on device
kelyfos: stopping the machine — it is not being recorded
```

`kelyfos run` then exits `1`, and the session's `session.end` reads
`recorder_failed`. Under `serve-mcp` the sandbox stops being served and the next
