...
*Written after F3, the security review of 2026-08-28: the channels now check who
is on the other end.* They serve the host — `exec` runs commands, `control` stops
the machine, MCP is the whole tool surface — and they are bound on
`VMADDR_CID_ANY`, because a guest's own CID can change across a snapshot restore
and a listener pinned to the old one would quietly stop accepting. `Accept` read
the peer's CID into a remote address and then never looked at it, so any peer
that could reach the socket was served as though it were the operator.

Nothing inside the guest can be that peer, and that was measured on a booted
guest rather than assumed: `vsock_loopback` is absent from `/proc/kallsyms`, and
a `connect` to `VMADDR_CID_LOCAL` times out. But a confined process *can* create
an `AF_VSOCK` socket — `socket` is not on the seccomp refusal list and will not
be, since a denylist that refuses `socket` refuses the network — so the only
thing in the way was one kernel config symbol, and
`net/vmw_vsock/Kconfig` marks that symbol **`default y`** (checked in 6.12.105,
the kernel this image ships, and in 6.19.14). The vendored base config turns it
off *against* the upstream default, and `check-kernel.sh` verifies the two
KelyfOS fragments and not the base — so an `olddefconfig` over a kernel bump
would have turned it back on with nothing to notice.

Both halves are closed. `kelyfos.fragment` pins
`# CONFIG_VSOCKETS_LOOPBACK is not set`, which puts it inside what
...
kernel offers root minus the names on that list — forty of them since the
...
into the output; and the serial number is a digest of the whole document, so two
documents over one component list — or two differing only in a licence, a CPE or
the hash of a source tarball — cannot share the identifier that exists to tell
them apart.
...
build machine. Components this tool did not author now pass through whole,
