...
## 4. The eight doors that open a chain

"Opens a chain" means: appends a `session.start`. This is the same criterion
`Event.WithPosture`'s call sites already use, and the two enumerations agree —
audited against the current source rather than assumed:

| # | Door | `session.start` site | `WithPosture` site |
| --- | --- | --- | --- |
| 1 | `kelyfos run` | `host/run.go:570` | `host/run.go:624` |
| 2 | `kelyfos team up` (every agent, cold or forked, declared or runtime-spawned) | `host/team.go:276` (once per team) | `host/team.go:509` (once per agent) |
...
**Four things that do *not* open a chain, on purpose, so a future ninth
door does not get invented by accident:**
...
- **`kelyfos resume`** calls `sandbox.Restore` — a real new VMM process — but
  ...
  of the machine rather than one per resume." §8.3 covers why it still gets
...
Eight fields are not new: `mem_mib`, `vcpu_count` and `cpu_quota_percent`
already exist on `Event` (used today by `resource.oom` and `resource.summary`)
and carry the exact same meaning here — the cap, not a measurement of it — so
`session.policy` reuses them rather than adding a fourth spelling of "the RAM
ceiling" to the struct. Everything else below is new.

| JSON key | Go field | Go type | Meaning | Present when |
| --- | --- | --- | --- | --- |
| `vcpu_count` *(existing)* | `VcpuCount` | `int` | `cpus` — cores the guest sees | always |
| `mem_mib` *(existing)* | `MemMiB` | `int` | `mem` — guest RAM cap, MiB | always |
| `cpu_quota_percent` *(existing)* | `CPUQuota` | `int` | `cpu_quota` — host CPU time, percent of one core; 0 when uncapped | always |
| `disk_bytes` | `DiskBytes` | `int64` | `disk` — ceiling on the packed workspace image | a workspace is attached |
| `scratch_bytes` | `ScratchBytes` | `int64` | `scratch` — tmpfs size behind the overlay | always |
| `net_mbps_rx` | `NetMbpsRx` | `int` | inbound rate cap, decimal Mbps; 0 when unthrottled | network is attached |
| `net_mbps_tx` | `NetMbpsTx` | `int` | outbound rate cap, same units | network is attached |
| `disk_iops` | `DiskIOPS` | `int` | block device operations/sec cap; 0 when unthrottled | always |
| `disk_mbps` | `DiskMbps` | `int` | block device bytes/sec cap; 0 when unthrottled | always |
| `max_runtime_ms` | `MaxRuntimeMS` | `int64` | wall-clock budget, milliseconds; 0 when unbudgeted | always |
| `idle_timeout_ms` | `IdleTimeoutMS` | `int64` | idle budget, milliseconds; 0 when unbudgeted | always |
| `allow` | `Allow` | `[]string` | the resolved egress allowlist | network is attached |
| `ports` | `Ports` | `[]int` | ports the allowlist actually covers — `[80, 443]` when unset and `allow` is non-empty (`egress.Policy.Ports`'s own default, P7-4) | network is attached |
| `secrets` | `Secrets` | `[]EvSecret` | bound credentials, by name, host and path scope — never a value (§8.1) | one or more are bound |
| `workspace` | `Workspace` | `string` | resolved host directory attached at `/work` | a workspace is attached |
| `plugins` | `Plugins` | `[]string` | configured plugin names (§8.2) | one or more `[[plugin]]` entries |
| `forwards` | `Forwards` | `[]string` | `"<host-port>:<guest-port>"` per `[[forward]]` entry | one or more are configured |
| `rootfs_sha256` | `RootfsSHA256` | `string` | the image manifest's `rootfs_sha256` | always |
| `kernel_sha256` | `KernelSHA256` | `string` | the image manifest's `kernel_sha256` | always |
| `tools` | `Tools` | `[]string` | the outward verbs usable against this machine (§8.3) | always |
| `parent_session` | `ParentSession` | `string` | the session id this machine was forked or restored from | fork, snapshot restore, `sandbox_restore`, `sandbox_fork` |
| `traceparent` | `Traceparent` | `string` | inbound W3C `traceparent`, verbatim, unparsed | `serve-mcp`, when the caller supplied one |
...
| --- | --- | --- | --- |
| `agents` | `Agents` | `[]EvAgent` | every resolved agent: name, its own sandbox id, its fork-template group |
| `edges` | `Edges` | `[]string` | the resolved, expanded pairs — reuses the exact `"from -> to"` strings `host/teamplan.go:126,142` already computes as `plan.edgeText`, post-glob-expansion |
| `store_keys` | `StoreKeys` | `[]EvStoreKey` | every `[[team.store.key]]` rule: its name/glob, its read list, its write list |
| `cpu_quota_percent` *(existing)* | `CPUQuota` | `int` | the collective slice's cap — `[team.resources] cpu_quota`; 0 when the team has no shared cgroup |
| `record_payloads` | `RecordPayloads` | `*bool` | whether `[team] record_payloads` is set — a pointer, like `Jailed` and `Overlay`, so `false` is distinguishable from "not a team" |
...
`secrets` carries `Name`, `Host` and `Path` — exactly what `secret.use`
already writes (`internal/recorder/schema.go:139`) — and nothing else.
`egress.Secret.value` (`internal/egress/secret.go:18`) is unexported and has
no accessor; there is no field on `EvSecret` for it to flow into even by
mistake. A secret bound and never used still appears in `session.policy`, by
name and host, which is D59's whole point: a policy ceiling nobody can see
being enforced is a ceiling nobody can audit. It appears with no value, which
is every other rule in this product about what a record may hold.
...
rather than implied. `tools` is drawn from two fixed lists, chosen by which
kind of door created the machine, and does not vary per policy today (nothing
in `kelyfos.toml` can currently narrow it further):

- **CLI-facing machines** (`run`, `team up`, `fork`, `snapshot restore`,
  `shim`): `["exec", "shell", "diff", "snapshot", "fork", "forward"]`, plus
  `"mcp"` when `[[plugin]]` is configured for that machine (guest-side MCP
  passthrough).
- **`serve-mcp`-facing machines**: the tool names that target an existing
  sandbox — `["sandbox_exec", "sandbox_read_file", "sandbox_write_file",
  "sandbox_stop", "sandbox_snapshot", "sandbox_restore", "sandbox_fork"]`
  (`host/servemcptools.go:31-150`) — never `sandbox_run` or `sandbox_list`,
  which do not name an existing machine.
...
### 9.2 Field order is append-only, and this document is not the last word on it

The order in §5 and §6 (session.policy's nineteen new fields, then
team.topology's four) is this document's proposal for the order P7-2 and
P7-3 append in — reusing the eight-cap block, then network, then identity,
then topology — but the only rule that is actually load-bearing is
**append at the end, never insert, never reorder**
(`TestTheEventFieldOrderIsFrozen`,
`internal/recorder/fieldorder_test.go:28`). If P7-2's implementation finds a
reason to sequence its own nineteen fields differently, that is a smaller
change than this document getting the *set* of fields wrong, and the
field-order test — not this page — is what has to agree afterwards.
...
enumerates the eight doors in §4 and fails when a ninth writes a
`session.start` without a matching `session.policy` — the same shape
`WithPosture` exists to enforce, for the reason P5-1's gap (jailed set at one
door, silent at seven) is a landmine entry in the agent brief this document's
author was given. `docs/reference/events.md` regenerates from `schema.go`
automatically (`make docs`); `docs/events.md`'s own per-type prose tables do
not — they are still hand-maintained (`docs/README.md`'s own inventory says
so), so the two new `### session.policy` / `### team.topology` sections there
are a manual edit P7-2/P7-3 has to remember, not something `make docs`
supplies.
