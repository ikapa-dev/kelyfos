...
| after something that works, right now | [`cookbook.md`](cookbook.md) — seventeen recipes, each one runnable as it stands |
...
| [`cookbook.md`](cookbook.md) | recipes | Seventeen complete, copy-pasteable recipes. Every one is a script CI extracts and runs on a real machine. |
...
### `cookbook.md` — seventeen things that work
...
is actually running (`team graph`, `team ps --graph`); and exporting a report
against a team that has not stopped, `--refresh` rewriting it atomically on a
clock so an open browser tab follows along through its own
`<meta http-equiv="refresh">` — checked for the one property that matters,
that the process doing the rewriting holds no socket at all.
