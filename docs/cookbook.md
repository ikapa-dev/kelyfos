...
kelyfos log | grep 'secret .*GITHUB_TOKEN -> api.github.com' >/dev/null
kelyfos log | grep 'egress allowed api.github.com:443 *mode=terminated' >/dev/null
kelyfos log | grep 'egress BLOCKED example.com:443' >/dev/null
kelyfos log --verify
```
...
cd "$work"
forks=""
# This recipe's own machines only. `pkill -f "kelyfos run"` is a host-wide
# question answered with a kill: on a machine somebody else is also using it
# stops their sandboxes too (P7-16). The `run` below is the trailing-command
# form, which tears its own machine down and exits, so the only thing to hold
# is the backgrounded `fork`.
trap '{ [ -n "$forks" ] && kill -INT "$forks" 2>/dev/null; } || true;
      wait "$forks" 2>/dev/null || true; rm -rf "$work"' EXIT
...
trap 'kelyfos team down ${SESSION:+--team "$SESSION"} >/dev/null 2>&1 || true;
      { [ -n "$team" ] && kill "$team" 2>/dev/null; } || true; rm -rf "$work"' EXIT
...
kelyfos verify --replay doctored.html | grep "a result" >/dev/null
...
if command -v openssl >/dev/null; then
  ...
  kelyfos verify signed.html --key signing.pub | grep "signed by the key you named" >/dev/null
  ...
  kelyfos verify report.html | grep "not signed, which is not a fault" >/dev/null
...
shim=$!
# `kill` on a shim that has already exited fails, and a plain failing command
# in an EXIT trap under `set -e` aborts the rest of it — so without `|| true`
# the working directory outlives the recipe on exactly the path that already
# failed.
trap 'kill $shim 2>/dev/null || true; wait $shim 2>/dev/null || true; rm -rf "$work"' EXIT
...
kelyfos connect claude-code --check 2>&1 | grep "speaks MCP" >/dev/null
...
echo "$refusal" | grep '\[\[plugin\]\] has no effect inside a team' >/dev/null
...
# This recipe's own team only — see recipe 5 for why (P7-16).
trap '{ [ -n "$REFRESH_PID" ] && kill "$REFRESH_PID" 2>/dev/null; } || true;
...
VIEW_PID=""
SSE_PID=""
# Every line braced and defaulted. `[ -n "$X" ] && kill "$X"` is exempt from
# `set -e` when X is empty — the failure is then a non-final element of the &&
# list — but not when X is set and its process has already gone, which is the
# state this recipe's own failure branch leaves. The trap would abort there and
# the working directory would stay.
trap '
...
trap 'kelyfos team down --team "${A_SESSION:-alpha}" >/dev/null 2>&1 || true;
      ...
      { [ -n "$A_UP" ] && kill "$A_UP" 2>/dev/null; } || true;
      { [ -n "$B_UP" ] && kill "$B_UP" 2>/dev/null; } || true; rm -rf "$work"' EXIT
...
if kelyfos doctor 2>&1 | grep "orphaned instances     none" >/dev/null; then
...
`head -N`, and test for a line with `grep pattern >/dev/null` rather than
`grep -q`, which exits at the first match the same way. Under `set -o
pipefail`, `head` exiting early sends SIGPIPE to
