...
| the supervisor is non-dumpable, so ptrace-shaped attacks are refused by the policy and the flag, not only the kernel ACL (audit A17b) | confinement battery (process_vm/pidfd probes EPERM); prctl failure would log at boot | CHECKED |
...
| plugin stderr is sanitised at the console boundary (audit A18) | `supervisor/pluginhost_sanitize_test.go` — escape sequences quoted whole, clean lines untouched, and a structural check pins the call site | CHECKED |
| the accepted read residuals (/dev/kmsg, raw device reads, read-only ioctls, /proc/1/environ) are named with their reasons, and the refusal-list counts live in tests rather than prose (audit A19) | docs/threat-model.md §4's residual list; the counts are enforced by `supervisor/profile_policy_test.go` and the confinement battery | CHECKED — documented |
