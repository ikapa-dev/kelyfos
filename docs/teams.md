...
print the identical topology, except in the one case §8.4.1 names — a
store enabled with no `[[team.store.key]]` rule, which the recorded
`team.topology` cannot currently distinguish from the store being off at
all. It also lists every refusal recorded since
