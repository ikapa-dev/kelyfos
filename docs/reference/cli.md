...
| [`kelyfos sessions rm`](#kelyfos-sessions-rm) | list paused sessions; rm, prune or erase one (-h on each for more) |
| [`kelyfos sessions prune`](#kelyfos-sessions-prune) | list paused sessions; rm, prune or erase one (-h on each for more) |
| [`kelyfos sessions erase`](#kelyfos-sessions-erase) | list paused sessions; rm, prune or erase one (-h on each for more) |
...
| Flag | Type | Default | Meaning |
| --- | --- | --- | --- |
| `--dry-run` | boolean | — | report what would be pruned without deleting anything |
| `--policy` | string | the nearest one, found by walking up | the kelyfos.toml whose [sessions] retention_days applies |

## kelyfos sessions erase
...
| Flag | Type | Default | Meaning |
| --- | --- | --- | --- |
| `--reason` | string | — | why — recorded in the session.erasure event this writes (required) |
